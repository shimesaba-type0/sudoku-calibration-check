...
- **確信度順モード**(Issue #38)では、周の対象マスを左上から順に聞くのではなく、Worker の `ask:"cell"`(4章)で「その周でまだ確定していないマスのうち、最も確定しやすいマス」を先に選ばせ、選ばれたマスを `ask:"digit"` で聞く。1マスあたりの API 呼び出しが2回になる。周の区切り・集計・15周の安全弁は左上から順のときと同じ。**Worker 側(`ask`)のみ実装済みで、フロントの順番トグルは Issue #38 の後半**
