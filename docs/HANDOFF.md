...
- 要オーナー判断(未対応。日付なしの定常項目): (1) 確信度順は 1 マス 2 回呼ぶので、1 周ぶん(約 102 回)で IP 上限 120 回/時をほぼ使い切り、1 問(約 156 回)は 1 時間で完走できない。上限を緩めるか(作業ルール 4)→ **#20**(追記済み)。(2) Claude 経路の実キー疎通(CORS ヘッダー、structured outputs + thinking、Haiku の budget_tokens)はオーナーのブラウザで確認が必要 → **#44**(手順あり)
