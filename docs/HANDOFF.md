...
- 残っている人間作業: 1.5(デフォルトブランチを `main` にする)。オーナー判断待ち: IP 単位のレート制限(30 回/時)の緩和可否(`CLAUDE.md` 現在の状態を参照)、Issue #10 の方針
- 次にやること: マージ後の `main` をデプロイし、ブラウザで実際に 1 問実行して SPEC 6 章の「デプロイ後」項目を確認する
