...
- 2026-09-22: #43 実装(未マージ・未デプロイ、PR 前)。Jev 経路(Worker)の 429/503、Claude 経路の 429/529/5xx・ネットワーク失敗を `pauseForTransientError()` で「停止」(#32 と同じ後始末)扱いにし、`errorMessage` を立てず `state.pauseReason` に理由を持たせた。それ以外(400/401/415/502/形式不正など)は従来どおりエラー停止。振る舞いテスト Y1〜Y5、E2E モック `--mode ratelimit` の [4] を更新
