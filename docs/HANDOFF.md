...
- 次にやること: オーナーがブラウザで 1 問実行して較正図を確認する(SPEC 6 章の「デプロイ後」項目の目視分)。拡張 3(Claude API)・4(ログ異常検知への転用)は依頼があってから Issue を起こす。知見のメモ: 同じ入力でも Jev の `choice` は揺れ、`probabilities` はほぼ平坦(最大 0.2 前後)なので、15 周で収束しないことが多い。較正図は `pc` と `conf` の 2 枚を比べるのが実験の核心
