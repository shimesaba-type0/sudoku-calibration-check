...
- 2026-09-22: 3 件を順にマージ・デプロイ。#39(Claude API、BYOK。Opus レビューの must-fix「adaptive thinking の max_tokens」を反映)→ Version `9feebd7b`。#40(`/api/judge` の `ask:"cell"`。Worker 側)→ Version `540034b3`。本物の Jev で 51 択のマス選びが 200 / 約 2 秒 / 確率合計 1.0 で返ることを確認(分布はほぼ平坦、最大 0.07)。#41(確信度順モードのフロント。Opus レビューの must-fix「E2E [7] の総数一致が mixed モードで不安定」を反映)→ Version `51abedb7`。実ブラウザ(Playwright + モック)で 2 段ループ・ヒートマップ・停止/再開を確認
- 2026-09-22: 運用メモ。セッションのプロキシ経由だと Playwright の本番モード(`npm run e2e -- --url`)が `ERR_TOO_MANY_RETRIES` で落ちることがある(1 リクエスト 5〜8 秒かかり、2 件目以降で失敗)。Worker 側は 200 を返しており(curl は 1〜2 秒)、コードの問題ではない。再実行で通ることもある。確実に見たいときは本番の HTML と `/api/judge` を curl で中継するローカルサーバー越しに Playwright を走らせる(`scripts/e2e/out/` は gitignore なので手順だけ: GET / と POST /api/judge を `curl` で本番に転送する `http.createServer` を立て、そこに `page.goto`)
- 要オーナー判断(未対応): (1) Claude 経路で Anthropic 側の 429 / 529 のときに盤面を捨てず「停止」扱いにして再開できるようにするか(SPEC F5「エラー時の復帰はリセットのみ」を変える話。PR #39 レビュー S4)。(2) 確信度順は 1 マス 2 回呼ぶので IP 上限 120 回/時でほぼ 1 周。上限を緩めるか(作業ルール 4)。(3) Claude 経路の実キー疎通(CORS ヘッダー、structured outputs + thinking、Haiku の budget_tokens)はオーナーのブラウザで確認が必要
