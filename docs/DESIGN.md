...
| `renderUsagePanel()` | 「この実行の消費」パネル(SPEC F1、Issue #55・#56。オーナー追加要望 2026-09-23)。`renderPricesPanel()` のすぐ上に置く(埋め込みモードでは `renderStats()` の直前)。トークン(`totalTokensIn` / `totalTokensOut`)・コスト(`formatUsdForModel(totalCostUsd, activeModelIdForPricing())`)・速度(`formatTps(totalTokensIn + totalTokensOut, totalLatencyMs)`。**レビュー S1**: 分母に `currentTotalElapsedMs()` を使うと速度モード/順番モードの演出待ち(`SLOW_BEFORE_COMMIT_MS` 等)が混ざって数倍ぶれるため、API 呼び出しの待ち時間の合計 `totalLatencyMs` だけを使う。演出待ちを含まない旨を表示にも添える)の3行 |
