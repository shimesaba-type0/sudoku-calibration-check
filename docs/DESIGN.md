...
- **一括モードの1周ぶん(Issue #48)**: `askAllClaude(puzzle, signal)` が
  ...
  - **チャンク分割(Issue #74)**: `buildClaudeAllSchema` はマス数ぶん独立した入れ子オブジェクトを
    ...
    - **部分失敗の扱い(PR #75 レビュー S1)**: `Promise.all` ではなく `Promise.allSettled` を使う。
      ...
      半分ぶんの usage(`err.innerPartialUsage`)もここで合算する。Issue #93)。`askAllRound()` の
      ...
      制約。成功済みチャンクをキャッシュして失敗したチャンクだけ再送信する設計にすれば防げるが、
      実装コストに対して見合わないと判断し見送った)。エラー停止(`showError()`。429/529/5xx・
      ネットワーク失敗以外の非 transient な失敗)の場合は復帰がリセットのみなので、
      `carryUsage` に積んでも次の commit で消費される前に `reset()` が空にする(実害は無いが、
      一貫性のため transient/非 transient を問わず同じ経路で積む)
    - 戻り値の `cells` は成功したチャンクの結果をマージしたもの、`usage` は
      `combineClaudeUsage(a, b)`(`extractClaudeUsage()` と同じ形
      `{ input_tokens, output_tokens, cache_read_input_tokens? }` を2つ合算する純粋関数。
      `combineRecordUsage`(4.2)の記録形版と同じ考え方だが、`toRecordUsage()` に通す前の生の
      usage を合算する必要があるので別に持つ)で成功したチャンクぶんを合算したもの、
      `chunkCount` はチャンク数。`request` は成功した先頭チャンクの `body`(失敗時は実際に
      失敗したチャンクの `body`)を返す(プロンプト枠は `renderAllRequestBlock()` が
      `allReq.chunkCount > 1` のとき、成功時は「(N回に分割。先頭の1回だけ表示)」、失敗時は
      「(N回に分割。失敗した1回を表示)」を添えて、表示しているのが全部ではないことを明示する)
    - **コスト・レート制限への影響**: チャンクごとに `CLAUDE_ALL_SYSTEM` と盤面全体(9行)を
      毎回送り直すため、51マスの固定問題(既定6チャンク)では一括の入力トークンがチャンク分割前
      よりおおむね6倍になる(`docs/SPEC.md` 5章)。また6リクエストが同時に飛ぶため、Anthropic
      側のレート制限に近いキーではバーストで 429 を踏みやすくなる可能性がある(その場合も上記の
      部分失敗の扱いにより、429 が来る前に成功していたチャンクぶんの usage は失われない)
    - Jev 経路(Worker、`ask:"all"`)はこの制約と無縁なので変更していない
...
| `setupEmbedMessageListener()` | 埋め込みモードの起動時に1回呼ぶ。`window.addEventListener("message", ...)` で親からの操作を受け、`event.origin === location.origin` のときだけ `run()` / `stop()` / `reset()` / (`newPuzzle` なら `applyPuzzleFromString()` → `reset()`) / `setSpeed()` / `setOrderMode()` / `setHistoryMode()` / `setRuleMode()`(Issue #61・#63)/ `setClaudeModel()`(Issue #90)を呼ぶ。`setClaudeModel()` は他と違い内部で `saveClaudeSettings()` を呼ぶが、`embedMode` では即 return するのでこの呼び出し経路は `localStorage` に触らない(下記) |
| `renderCompareShell()` / `compareRun()` / `compareStop()` / `compareReset()` / `compareNewPuzzle()` / `compareSetSpeed()` / `compareSetOrderMode()` / `compareSetHistoryMode()` / `compareSetRuleMode()` / `compareSetClaudeModel()` / `compareSetDifficulty()` / `updateCompareStatus()` / `setupCompareMessageListener()` | 比較シェル(`GET /compare`、Issue #46、8章)。`compareSetHistoryMode()` / `compareSetRuleMode()`(Issue #61・#63)は `compareSetOrderMode()` と同じロック条件(`compareEitherRunning() || compareEitherPaused()`)で `state.historyMode` / `state.ruleMode` を切り替え、両 iframe に `{ type: "setHistoryMode", on }` / `{ type: "setRuleMode", on }` を `postMessage` する。`compareSetClaudeModel()`(Issue #90)も同じロック条件で `state.claudeModel` を切り替え、`saveClaudeSettings()` で `localStorage` にも保存してから両 iframe に `{ type: "setClaudeModel", model }` を `postMessage` する(通常ページの `renderClaudeSettings()` が `embed=1` では描かれないため、モデル選択だけは比較シェル自身の上部バーに持たせた。API キー入力は引き続き比較シェルに置かず、通常ページの設定パネルで保存する)。**保存するのは比較シェル(親)だけ**: 両 iframe(embed 側)も `{type:"setClaudeModel"}` を受け取ると同じ `setClaudeModel()` を呼ぶが、`saveClaudeSettings()` は `embedMode` なら即 return するので `localStorage` には触らない。埋め込み側は `modelMode` が URL(`model=jev`/`model=claude`)で固定されているため、そのまま保存を許すと2つの iframe が交互に書き戻し合い、通常ページの保存済み `modelMode`(Jev/Claude のどちらを使うか)を意図せず上書きしてしまうため(PR #91 の Opus レビューで発見)。`renderCompareShell()` は `app.innerHTML` を**最初の1回だけ**組み立て(上部バー・2つの `<iframe class="compare-frame">`・各ステータス欄)、直後に `document.querySelectorAll(".compare-topbar" / ".compare-status" / ".compare-frame")` で要素を拾って `compareEls` / `compareFrames` に保持する。以後の更新(`compareRun()` 等)は `app.innerHTML` を触らず、`compareEls.topbar.innerHTML` / `compareEls.statusJev.innerHTML` / `compareEls.statusClaude.innerHTML` だけを差し替える(iframe の再読み込みを避けるため)。`comparePostToFrames(message)` が両 `iframe.contentWindow` に同じメッセージを `postMessage(message, location.origin)` する。`setupCompareMessageListener()` は子からの `status` メッセージ(`event.origin` と `event.source`(どちらの `contentWindow` か)を確認)を `updateCompareStatus()` に振り分ける。経過時間・コストの見出し(`renderCompareStatusHtml()`)は子が申告する `elapsedMs`/`costUsd` をそのまま使い、親側の計測(旧 `compareStartedAt` / `compareElapsedSeconds()`。Issue #55 で廃止)は持たない。`renderCompareStatusHtml()` は見出しの下に `renderPromptPanelBody()`(4.2「モデルに送ったプロンプト」)を呼んで直近のリクエストも表示する(Issue #80) |
...
- **見出し行の経過時間(Issue #80、オーナー要望 2026-09-23)**: 「現在の判定」の見出し
  ...
  右側に `panel-title-elapsed`(id 付き、`currentTotalElapsedMs()` を `formatMs()` で
  ミリ秒表示)を常時出す。以前はパネル本文の先頭に「経過 X ms / $Y」という行
  (`elapsedLine`)を実行中だけ出していたが、コストは「この実行の消費」パネル
  (`renderUsagePanel()`)と重複していたため、経過時間だけを見出し行に移してコストの
  重複表示を削除した(`head` の組み立て1か所にまとめたので、`isPaused()`/`allFetching`/
  フォーカス中/待機中の各分岐で個別に `elapsedLine` を差し込む必要も無くなった)。
  **リアルタイム更新(オーナー追加要望 2026-09-23)**: 当初は `render()` のたびにしか
  更新されず、一括モードの fetch 待ち中(数秒〜数十秒 `render()` が挟まらない)は
  「開始から終了までのトータル時間」が止まって見えた。`startElapsedTicker()` が
  `run()` で `setInterval(tickElapsedDisplay, ELAPSED_TICK_MS)`(100ms)を張り、
  `tickElapsedDisplay()` は `render()` を丸ごと呼ばず `#current-elapsed` の
  `textContent` だけを書き換える(`.top-controls` の `<select>` フォーカスガードや
...
- **比較モードのプロンプト可視化(Issue #80、オーナー要望 2026-09-23)**: 埋め込みモードは
  ...
  `render()` にある `oldDetails`/`detailsWasOpen` の仕組みは `document.querySelector()` で
  1個だけ拾う作りなので、同じ class の `<details>` が2つ(比較シェルの Jev/Claude 各カラム)
  ある場面は想定しておらず、2カラム分への拡張は今回のスコープ外。Issue #80)。
  **`renderComparePromptBody(info)`**
...
- **Claude 設定パネルの再構成(Issue #37、オーナー要望 2026-09-23)**: `renderClaudeSettings()`
  を、常に見える行(モデル選択・思考トグル)と、既定で閉じている `<details
  class="claude-key-details">`(API キーの入力欄・保存/削除ボタン・注意文)に分けた。
...
- 必ず含めるテスト
  ...
  - **比較モード**(`test/page.test.js` X1〜X6、Issue #46)。`runScript` の harness に `opts.location`(`{ pathname, search, origin }`、既定 `{ pathname:"/", search:"", origin:"https://example.com" }`)と `opts.window`(`{ parent, addEventListener, postMessage }`、既定は自分自身が `parent` の自己参照オブジェクト)を追加。`opts.document` は丸ごと差し替えられる(比較シェルの iframe / `querySelectorAll` を検査する X4 用)
