...
- 必ず含めるテスト
  ...
  - **比較モード**(`test/page.test.js` X1〜X5、Issue #46)。`runScript` の harness に `opts.location`(`{ pathname, search, origin }`、既定 `{ pathname:"/", search:"", origin:"https://example.com" }`)と `opts.window`(`{ parent, addEventListener, postMessage }`、既定は自分自身が `parent` の自己参照オブジェクト)を追加。`opts.document` は丸ごと差し替えられる(比較シェルの iframe / `querySelectorAll` を検査する X4 用)
