...
- 消去法(#61 履歴・#63 ルール候補)のフロントと統計カードの分割(#60)。「消去法(履歴): あり/なし」「ルール候補: あり/なし」の2トグル(既定あり、URL パラメータ `history=`/`rules=`、比較シェルにも同じトグル)。`excludeFor(r,c)` = 履歴(`wrongDigits`)とルール候補(`ruleExclusions(r,c)`、正解は見ない)の和集合を `exclude` として `/api/judge` / Claude に渡す。記録に `n`/`e`/`rc` が付く。統計カードは「この周の判定済み」「この周の正解」の2枚に分け、強制終了時は周回ログに専用の1行を足す。テストは振る舞いテスト AB1〜AB10
