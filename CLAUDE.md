...
- 一時的な失敗は停止扱い(#43、SPEC F5、オーナー判断済み): Jev 経路(Worker)の 429(レート制限)/503(カウンタ障害)、Claude 経路の 429/529/5xx・ネットワーク失敗は、`errorMessage` を立てず `pauseForTransientError()` で「停止」(#32 と同じ後始末)にする。盤面・周回ログ・queue は保たれ、「停止中」の下に理由(`state.pauseReason`)が出て「再開」で続きから動く。それ以外(400/401/415/502/形式不正・キー未設定など)は従来どおりエラー停止(復帰はリセットのみ)。振る舞いテスト Y1〜Y5 あり。E2E モック `--mode ratelimit` の [4] もこの停止扱いを検証する形に更新済み
