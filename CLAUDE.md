...
- `docs/HANDOFF.md` — 引き継ぎ手順とタスク用プロンプト
...
5. 変更後は必ず `npm run check` を通す。デプロイできる環境なら `npm run deploy` → `curl` で `/api/judge` を叩いて動作確認し、結果(レイテンシ・実際のレスポンスJSON)を報告に含める
...
- v0.1: 固定の数独1問(Wikipediaの例題)で、判定→採点→周回のループ、IP単位+全体のレート制限まで実装済み。**まだ一度も実環境で Jev を叩いていない**ので、最初のタスクは「本当に動くかの検証」
