...
- デプロイ済み: https://sudoku-calibration-check.takashi-kono-rb.workers.dev (2026-09-23 時点の Version は `35a0566f`。以後は `docs/HANDOFF.md` 6 章の最新の Version 記載を参照)。`npm run e2e -- --url <URL>` の本番モード(ヘッダー 3 つ + 最速で 3 判定)が PASS
...
- 処理時間(#55)とコスト表示(#56)のフロント: 「現在の判定」パネルの見出し行の右に開始から終了までの経過時間(ミリ秒。停止中は数えない)を常時表示する(#80。以前はパネル本文に「経過 X ms / $Y」の行があったが、コストは「この実行の消費」パネルと重複していたので経過時間だけ見出し行に移し、コストの重複表示は削除した)。周回ログにも周ごとの処理時間とコストを表示。完了/強制終了バナーにも開始から終了までの所要時間が出る(#76)。記録に `u`(トークン使用量)・`t`(呼び出しのレイテンシ)が付き(一括モードは周の先頭1件だけ、確信度順はマス選び+数字の合算)、`costOf()` が単価(既定値と `localStorage` の `scc.prices.v1`。編集 UI は #76 で削除したのでコードを直接編集する運用)から計算する。振る舞いテスト AA1〜AA13・AD4 あり。「この実行の消費」パネル(累計トークン・コスト・平均 tok/s の実測値)を盤面の直下に表示する(#76。当初は単価パネルの直上だったが、単価パネルごと削除して盤面の下に移した)
