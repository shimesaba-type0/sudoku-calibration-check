...
- デプロイ済み: https://sudoku-calibration-check.takashi-kono-rb.workers.dev (2026-09-23 時点の Version は `271fc56f`。以後は `docs/HANDOFF.md` 6 章の最新の Version 記載を参照)。`npm run e2e -- --url <URL>` の本番モード(ヘッダー 3 つ + 最速で 3 判定)が PASS
...
- 処理時間(#55)とコスト表示(#56)のフロント: 「現在の判定」パネルと周回ログに、周ごと/全体の走っている時間(ミリ秒。停止中は数えない)と累計コストを表示。完了/強制終了バナーにも開始から終了までの所要時間が出る(#76)。記録に `u`(トークン使用量)・`t`(呼び出しのレイテンシ)が付き(一括モードは周の先頭1件だけ、確信度順はマス選び+数字の合算)、`costOf()` が単価(既定値と `localStorage` の `scc.prices.v1`。編集 UI は #76 で削除したのでコードを直接編集する運用)から計算する。振る舞いテスト AA1〜AA13 あり。「この実行の消費」パネル(累計トークン・コスト・平均 tok/s の実測値)を盤面の直下に表示する(#76。当初は単価パネルの直上だったが、単価パネルごと削除して盤面の下に移した)
- UI レイアウト刷新(#76、オーナー要望 2026-09-23): 実行系ボタン+速度・難易度・モデル・順番・消去法(履歴・ルール候補)の6トグルを、完了バナーの下・盤面の上の横並びバーにまとめ、6トグルはボタン群から `<select>`(プルダウン)に変えた(比較シェル自身の上部バーはボタン群のまま対象外)。単価パネルを画面から削除(コスト計算自体は変えていない)。統計(5項目)はカードのグリッドから1行のコンパクト表示に。「現在の判定」→「周回ログ」を優先し、「モデルに送ったプロンプト」枠は周回ログの後ろに回した。完了/強制終了バナーに所要時間を追加
