...
- デプロイ済み: https://sudoku-calibration-check.takashi-kono-rb.workers.dev (2026-09-23 時点の Version は `46c63baf`。以後は `docs/HANDOFF.md` 6 章の最新の Version 記載を参照)。`npm run e2e -- --url <URL>` の本番モード(ヘッダー 3 つ + 最速で 3 判定)が PASS(セッションのプロキシ経由だとフォント読み込みなどで `ERR_TOO_MANY_RETRIES` が出て偽陽性の FAIL になることがある。`docs/HANDOFF.md` 6 章の回避策を参照)
...
- マージ済み PR: #7(Worker 本体)、#9(Jev 実レスポンス形式 + KV id)、#11(#7/#9 のレビュー指摘)、#13(フロントエンド)、#14(DO レート制限)、#15(ジェネレーター/ソルバー)、#22(モデル方針)、#23(`/api/status`)、#24(E2E スモーク)、#25(集計ビュー)、#26(README)、#27(AbortController + 振る舞いテスト)、#28(難易度)、#33(停止/再開)、#35(Jev に送ったプロンプトの表示枠)、#39(Claude API、BYOK)、#40(`/api/judge` の `ask:"cell"`)、#41(確信度順モードのフロント)、#47(`ask:"where"`)、#49(比較モード)、#50(E2E の CSP 追随)、#51(一時的な失敗の停止扱い)、#52(`ask:"all"`)、#57(`usage`)、#58(一括モードのフロント)、#62(バナーを最上部へ)、#65(`exclude`)、#67(処理時間・コスト)、#70(この実行の消費パネル)、#72(消去法のフロント・統計カード分割・M1 修正)、#75(一括モード Claude 経路のチャンク分割)、#78(UI レイアウト刷新)、#81(一括+最速の描画省略・現在の判定の経過時間・比較モードのプロンプト可視化)、#82(E2E 本番モードの `waitForFunction` タイムアウト修正)、#85(経過時間のリアルタイム表示・Claude設定のAPIキーUI分離と折りたたみ)、#88(難易度「上級」の追加)、#91(比較モードで Claude のモデルを選べるようにする)
- 数独ジェネレーター/ソルバー(#5)と難易度トグル(#21、やさしい 36 / ふつう 30 / むずかしい 25 ヒント)。Worker 側の入力検証から固定問題(`GIVEN`)前提の項目は外れている(`docs/DESIGN.md` 3.3)。**上級(#87、オーナー要望 2026-09-23。にこりの数独の上級相当)**: 目標22ヒント(実際には22〜27程度に収束)。ベンチマークの結果、ジェネレーター単体(`generatePuzzle()`、1回勝負の貪欲法)は目標をどれだけ下げても実質22〜29あたりが到達限界と判明したため、`MIN_TARGET_GIVENS` を理論上の最小値17に、`GENERATE_RETRIES` を3→20に変更した(上級の収束率向上が主目的。むずかしいは単発でも約85%の確率で25ちょうどに届くため影響は小さい)。生成時間はブラウザ実機で1回あたり平均60ms・最悪150ms程度で、既存の「生成中…」表示でブロッキングを隠す設計のまま。UXとして許容範囲かはオーナー判断待ち(`docs/HANDOFF.md` 6章)
...
- Claude(Anthropic API)との比較(#37、SPEC 7 章 拡張 3): 「Jev / Claude」トグルと設定パネル(API キー・モデル・思考の有無)。ブラウザから `api.anthropic.com` を直接呼び(BYOK、Worker は関与しない)、structured outputs で `choice` / `probabilities` / `confidence` を自己申告させる。記録に `m`(モデル識別子)が付き、較正図をモデルで絞り込める。**実キーでの疎通はオーナーのブラウザで確認する**(セッションにキーは無い)。**設定パネルの再構成(#84・#85)**: モデル選択・思考トグルは常に見え、API キーの入力欄・保存/削除ボタンは既定で閉じている `<details>` の中(見出しは「API キー(設定済み/未設定)」の状態だけ)。末尾4文字のヒント表示(`anthropicKeyHint()`)は不要という要望で関数ごと削除した
...
- 一括モード(#48、SPEC 3章 F1・F3): 「左上から / 確信度順 / 一括」の順番トグルに「一括」を追加。周ごとに1回だけ Worker の `ask:"all"`(Jev)/ 同種のスキーマ(Claude)を呼び、その周でまだ確定していないマス全部の `choice` をまとめて受け取る。応答はその周だけのキャッシュ(`allResults`)に置き、以降は「フォーカス→バー表示→確定→次へ」の流れを **fetch なしで** 1マスずつ回す(記録は従来どおり1マス1件)。周が変わる・リセット/新しい問題/エラーでキャッシュを空にし、停止/再開(手動・一時的な失敗とも)はキャッシュを保って再開時に再度呼ばない(呼び出し自体の最中に停止したときだけ呼び直す)。記録の `m` は `typesafe/jev/all`(Claude は `<既存のモデル識別子>/all`)、`o` は `"all"`。現在の判定パネルは呼び出し中「一括で判定中…(Nマス)」、プロンプト枠は「一括: 質問N問」の要約 + 折りたたみ。**1周1回なので回数は最少だが、1回の単価は `digit` の約14倍**(`docs/SPEC.md` 5章)。Claude 経路の `max_tokens` は思考なし16000・adaptive 24000・Haiku(budget 2048)12000を下限にする(既存設定との `Math.max`。実キーでの疎通は未検証、オーナーのブラウザで確認する必要あり)。振る舞いテスト Z1〜Z8・E2E [9] あり。**Claude 経路はチャンク分割(#74・#75)**: マス数が多いとスキーマが大きくなりすぎて Anthropic の構造化出力が「compiled grammar is too large」で 400 を返すため、`CLAUDE_ALL_CHUNK_SIZE`(既定10)件ずつに区切って並列に呼び、結果をマージする(`Promise.allSettled`。一部だけ失敗しても成功ぶんの usage は `carryUsage` に積んで失わない。再開時は全チャンク再送信になるため、成功ぶんの二重課金は既知の制約)。チャンクサイズが実キーで確かめた値ではない点は要オーナー判断(`docs/HANDOFF.md` 6章、#44)。振る舞いテスト Z9・Z10 あり。**描画省略トグル(#80・#81)**: 一括+最速のときだけ選べる「描画: 毎回表示 / 省略(最速)」トグル。「省略(最速)」なら応答が届いたあとの1マスずつの `render()` 演出(フォーカス→バー表示→確定→次へ)をせず、`commitAllInstant()` が周の残り全マスを一気に確定して1回だけ再描画する(記録・統計・消去法履歴などのロジックは変えず、演出だけを省く)。振る舞いテスト AD1〜AD6 あり
- 消去法(#61 履歴・#63 ルール候補)のフロントと統計カードの分割(#60、#72)。「消去法(履歴): あり/なし」「ルール候補: あり/なし」の2トグル(既定あり、URL パラメータ `history=`/`rules=`、比較シェルにも同じトグル)。`excludeFor(r,c)` = 履歴(`wrongDigits`)とルール候補(`ruleExclusions(r,c)`、正解は見ない)の和集合を `exclude` として `/api/judge` / Claude に渡す。記録に `n`/`e`/`rc` が付く。統計カードは「この周の判定済み」「この周の正解」の2枚に分け、強制終了時は周回ログに専用の1行を足す。**`ruleExclusions(r,c)` は `settledKeys`(周の開始時点でのスナップショット、前の周までに正解して確定したマスだけ)経由でしか実盤面を見ない**(今の周の中の推測は正解・不正解を問わず含めない)。当初 `state.values` を素通しで見ていたため、同じ周内で先に(不正解のまま)確定した推測が別マスの正解を除外しうるバグがあり、#72 の Opus レビューで見つけて修正した。テストは振る舞いテスト AB1〜AB14
- 既知の未実装: 局所文脈(#64)は次の実験。`ask:"where"` のフロントはオーナー判断で作らない(#45 は閉じた。Worker 側の API は残る)。SPEC 7 章の拡張 4(ログ異常検知への転用)は依頼があるまで着手しない。#36(盤面の表現の比較)は検討 Issue
...
- `/api/judge` の `exclude`(#61 の Worker 側、消去法)。`digit`(省略時)は外す数字の配列、`ask:"all"` はマスのキーごとの配列。criteria と応答検証(`probabilities` のキー・`choice`)を残りの数字だけにする。9 個全部・不正な要素・重複は 400、`cell` / `where` に付いていたら 400。送るのは外れた数字だけで正解は送らない(`docs/DESIGN.md` 9 章 不変条件 1 の例外)。フロント(履歴の保持・トグル)は上記「消去法」の行を参照
...
- 比較モード(#46、SPEC 3章 F1'): `GET /compare` が通常ページを iframe で2枚(Jev/Claude)並べ、上部バーから `postMessage` で同時に操作する。判定ループの `state`/`queue`/実行世代は2インスタンス化せず、通常ページ(`GET /`)側に `puzzle=` / `model=` / `order=` / `speed=` / `embed=1` の URL パラメータと埋め込みモードを追加して対応。CSP の `frame-ancestors` は `'none'` から `'self'` に変更。記録の追記は毎回 localStorage を読み直す(2 枚が同時に書いても失われない)。振る舞いテスト X1〜X4・E2E [8]。**Claude のモデル選択(#90・#91、オーナー要望 2026-09-23)**: 通常ページの Claude 設定パネルは `embed=1` では描かれないため、比較シェル自身の上部バーに `CLAUDE_MODELS` の3択ボタンを追加。`compareSetClaudeModel()` が両 iframe に `setClaudeModel` を `postMessage` する。**`saveClaudeSettings()` は `embedMode` なら即 return する**(埋め込み側の iframe は `modelMode` が URL で固定されているため、素直に保存を許すと2枚の iframe が交互に localStorage を書き戻し合い、通常ページの保存済み設定を壊しうるバグが Opus レビューで見つかった。保存は比較シェル(親)だけが行う)。振る舞いテスト X5・X6
...
- 処理時間(#55)とコスト表示(#56)のフロント: 「現在の判定」パネルの見出し行の右に開始から終了までの経過時間(ミリ秒。停止中は数えない)を常時表示する(#80。以前はパネル本文に「経過 X ms / $Y」の行があったが、コストは「この実行の消費」パネルと重複していたので経過時間だけ見出し行に移し、コストの重複表示は削除した)。**リアルタイム更新(#84・#85)**: `setInterval(100ms)` で `render()` を待たずに `#current-elapsed` を直接書き換えるので、一括モードの応答待ち中など `render()` が挟まらない間も止まらない。比較シェルの見出しの経過時間も、子(iframe)がティックから10回に1回(≒1秒に1回)`postStatus()` するので同様に止まらない。周回ログにも周ごとの処理時間とコストを表示。完了/強制終了バナーにも開始から終了までの所要時間が出る(#76)。記録に `u`(トークン使用量)・`t`(呼び出しのレイテンシ)が付き(一括モードは周の先頭1件だけ、確信度順はマス選び+数字の合算)、`costOf()` が単価(既定値と `localStorage` の `scc.prices.v1`。編集 UI は #76 で削除したのでコードを直接編集する運用)から計算する。振る舞いテスト AA1〜AA13・AD4 あり。「この実行の消費」パネル(累計トークン・コスト・平均 tok/s の実測値)を盤面の直下に表示する(#76。当初は単価パネルの直上だったが、単価パネルごと削除して盤面の下に移した)
- UI レイアウト刷新(#76、オーナー要望 2026-09-23): 実行系ボタン+速度・難易度・モデル・順番・消去法(履歴・ルール候補)の6トグルを、完了バナーの下・盤面の上の横並びバーにまとめ、6トグルはボタン群から `<select>`(プルダウン)に変えた(比較シェル自身の上部バーはボタン群のまま対象外)。単価パネルを画面から削除(コスト計算自体は変えていない)。統計(5項目)はカードのグリッドから1行のコンパクト表示に。「現在の判定」→「周回ログ」を優先し、「モデルに送ったプロンプト」枠は周回ログの後ろに回した。完了/強制終了バナーに所要時間を追加。**描画省略トグルの追加で7トグルに(#80・#81)**
- 比較モードのプロンプト可視化(#80・#81、オーナー要望 2026-09-23): `/compare` の埋め込み iframe は「モデルに送ったプロンプト」パネルを描かない設計(SPEC F1')なので、「Claude が動いているか分からない」状態になっていた。`postStatus()`(子→親)に直近のリクエスト(`orderMode`/`lastRequest`/`lastRequestFailed`/`lastCellRequest`/`lastAllRequest`)を乗せ、比較シェルの `renderCompareStatusHtml()` が `renderPromptPanelBody()`(通常ページと共通、二重管理しない)で各カラムに同じ内容を表示する。比較シェルだけ、折りたたみの無い左上から/確信度順モードの生の `<pre>` も `renderComparePromptBody()` で折りたたみ、Jev/Claude の応答タイミングのずれで2カラムの縦幅が食い違い iframe がずれる問題を防いでいる(開閉状態の引き継ぎは対象外)。振る舞いテスト AD5 あり
