...
- デプロイ済み: https://sudoku-calibration-check.takashi-kono-rb.workers.dev (2026-09-22 時点の Version は `92a11dd4`。以後は `docs/HANDOFF.md` 6 章の最新の Version 記載を参照)。`npm run e2e -- --url <URL>` の本番モード(ヘッダー 3 つ + 最速で 3 判定)が PASS
...
- 既知の未実装: 処理時間(#55)とコスト表示(#56)のフロント(次の PR。Worker 側の `usage` は #57 で済み)。`ask:"where"` のフロントはオーナー判断で作らない(#45 は閉じた。Worker 側の API は残る)。SPEC 7 章の拡張 4(ログ異常検知への転用)は依頼があるまで着手しない。#36(盤面の表現の比較)は検討 Issue
