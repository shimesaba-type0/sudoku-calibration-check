...
(「合計 12,345 ms / $0.0012(3周)」)が1行足されます。**処理時間は「停止」している間は
数えません**(停止 → しばらく放置 → 再開、しても停止中の時間はカウントされない)。

「この実行の消費」パネルには、今の実行(リセットするまで)の累計トークン数・累計コスト・
平均速度(tok/s)が実測値でそのまま出ます。「単価」パネルはレートの**設定**であって実消費
ではないので、混同しないよう別枠にしてあります。

コストの計算に使う単価(100万トークンあたり USD)は、設定パネル群の「単価」で編集できます
(Jev `typesafe/jev` 入力 `0.042` / 出力 `0`(無料)、`claude-opus-5` 入力 `5` / 出力 `25`、
`claude-sonnet-5` 入力 `2` / 出力 `10`、`claude-haiku-4-5` 入力 `1` / 出力 `5` が既定値。
「既定値に戻す」ボタンあり)。単価は表示計算にだけ使い、`localStorage`(`scc.prices.v1`)に
保存されます。単価が設定されていないモデルの表示には「(単価未設定)」が付きます。
一括モードは1回の応答で周ぶんのトークン数が返るので、周の記録のうち先頭1件にだけコストを
乗せます(二重計上防止)。確信度順モードはマス選び+数字の2回ぶんのトークン数を合算します
