...
画面の「Jev / Claude」トグルで Claude を選ぶと設定パネルが出ます。常に見えるのはモデル
(`claude-opus-5` / `claude-sonnet-5` / `claude-haiku-4-5`)と思考の有無の選択だけで、
**API キーの入力欄は「API キー(未設定/設定済み)」の見出しをクリックして開く折りたたみの中**に
あります(普段は見なくてよいので既定で閉じています)。**自分の Anthropic API キー**を入れて
保存すると、ブラウザが `https://api.anthropic.com/v1/messages` を **直接** 呼びます
(この Worker は関与せず、キーも受け取りません)。キーはそのブラウザの `localStorage` にだけ保存され、
「キーを消す」で削除できます(画面にはキーの値そのものも末尾のヒントも出しません)。利用料は
自分の Anthropic アカウントに課金されます。較正図はモデルごとに絞り込めます。1 問あたり約 78 回
呼ぶので、Opus 5 + 思考ありでは 1 問で相応の料金になります(この経路に Worker のレート制限は
効きません)。確率は structured outputs で Claude 自身に申告させたものです
...
順番=一括・速度=最速のときだけ、隣に「描画」トグル(「毎回表示」/「省略(最速)」)が選べます。
「省略(最速)」にすると、応答が届いたあとの1マスずつのバー表示→確定の演出を省き、その周の
残り全マスを一気に確定してから1回だけ画面を更新します。呼び出し回数(1周1回)自体は変わらず、
届いた結果を画面に反映する演出だけを省くので、記録・較正への影響はありません(`docs/SPEC.md`
3章 F1、Issue #80)。
...
「現在の判定」パネルの見出し行の右に、開始から終了までの経過時間(ミリ秒、例「3,214 ms」)が
常時出ます(累計コストは下記の「この実行の消費」パネルに一本化しています)。この時間は
100ミリ秒ごとにリアルタイムで動き、一括モードの応答待ちなど盤面の描画が挟まらない間も
止まりません。周回ログの各行にも、その周だけの処理時間とコストが付きます
...
出ます(子ページが申告した値をそのまま表示、親側で計測し直しません)。埋め込みの iframe 自体は
「モデルに送ったプロンプト」パネルを描かないため、見出しの下に直近のリクエスト(実際に API へ
渡した入力の有無・中身)を同じ内容で表示し、「Claude が動いているか分からない」状態にならない
ようにしています(Issue #80)。詳細は `docs/SPEC.md` 3章 F1' / `docs/DESIGN.md` 8章。
